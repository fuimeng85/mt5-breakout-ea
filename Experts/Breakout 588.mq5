...
// [修改4] 取消MODE1与MODE2的所有互斥，各自跑各自的逻辑
void ProcessTFEntry_Mode1Mode2(ENUM_TIMEFRAMES tf, int dir)
{
   ...
   // 1. 独立运行 Mode 2 (BreakScan)
   if(TF_UseBreakScan(tf) && g_mode2Enabled)
   {
      TryEntryOnTF_BreakoutScan(tf, dir);
   }

   // 2. 独立运行 Mode 1 (普通顺势MACD)
   if(InpUseMode1)
   {
      TryEntryOnTF(tf, dir);
   }
...
int OnInit()
{
   ...
   Print("Donchian_MACD v4.99 - Independent 3 Modes Enabled");
